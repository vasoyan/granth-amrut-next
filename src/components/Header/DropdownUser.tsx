...
const DropdownUser = () => {
  ...
  useEffect(() => {
    setUser(authContext?.user); // Get UserData from context API
  }, [authContext]);

  const onSingout = async () => {
    try {
      const response = await ApiClient.get<null>("singout"); // Explicitly request null type

      if (response.success) {
        // console.log("Singout successful:", response.message);
        router.push("/signin");
      } else {
