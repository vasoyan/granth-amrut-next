...
export default function EditPage({ params }: { params: { id: number } }) {
  ...
  const fetchData = async (id: number) => {
    setIsLoading(true);
    try {
      const response = await ApiClient.getById<DailyDetail>("daily-detail", id);
      ...
      if (response.success && response.data && !Array.isArray(response.data)) {
        ...
        setCheckboxes({
          ...
          flagthal: response.data.flagthal ?? false,
          flagavd: response.data.flagavd ?? false,
          flagtelegram: response.data.flagtelegram ?? false,
          flagpatrikaapp: response.data.flagpatrikaapp ?? false,
          flagparamrutapp: response.data.flagparamrutapp ?? false,
          flagvicharanapp: response.data.flagvicharanapp ?? false,
          flagsuhradapp: response.data.flagsuhradapp ?? false,
          flagprivate: response.data.flagprivate ?? false,
          flagcalendar: response.data.flagcalendar ?? false,
          flagai: response.data.flagai ?? false,
        });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const response = await ApiClient.put<DailyDetail>(
      `daily-detail/${params.id}`,
      combinedData
    );
