...
export default function DailyDetailsPage({
  ...
  const [dailyDetailData, setDailyDetailData] = useState<DailyDetail[]>([]);
  ...
  const fetchData = async (userid: number) => {
    setIsLoading(true);
    try {
      const response: ApiResponse<DailyDetail[] | null> =
        await ApiClient.getByUserId<DailyDetail[]>("daily-detail", userid);
  ...
  const handleDelete = async () => {
    if (detailId > 0) {
      try {
        const response = await ApiClient.delete(`daily-detail/${detailId}`);
  ...
  const handleEdit = (id: number) => {
    router.push(`/daily-details/edit/${id}`);
  };
      ...
      <div className="rounded-sm border border-stroke bg-white px-5 pb-2.5 pt-6 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
        ...
        <div className="max-w-full overflow-x-auto pt-5">
          <table className="w-full table-auto">
            ...
            <tbody>
              {dailyDetailData.map((details, key) => (
                <tr key={key}>
                  ...
                  <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                    <p className="text-black dark:text-white">{details.city}</p>
                  </td>
                  <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                    <p className="text-black dark:text-white">
                      {details.flagcalendar ? "Yes" : "No"}
                    </p>
                  </td>
                  <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                    <div className="flex items-center space-x-3.5">
                      {authContext?.user?.userid == 1 ||
                      authContext?.user?.userid == userid ? (
                        <button>
                          <svg
                            className="hover:text-primary"
                            onClick={() => handleEdit(details.infoid)}
                            width="18"
                            height="18"
                            viewBox="0 0 24 24"
                            fill="none"
                      ...
                      {authContext?.user?.userid == 1 && (
                        <button
                          className="hover:text-primary"
                          onClick={() => handleOpenModal(details.infoid)}
                        >
                          <svg
                            className="fill-current"
                            width="18"
